...
//                                      !(pindexBIP34height->GetBlockHash() == chainparams.GetConsensus().BIP34Hash));
    ...
    if (pindex->nTime > sporkManager.GetSporkValue(SPORK_8_INDEXNODE_PAYMENT_ENFORCEMENT) && !IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
...
bool static ProcessMessage(CNode *pfrom, string strCommand,
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        int minPeerVersion = nHeight + 1 < chainparams.GetConsensus().nSeperateCalcDiffHeight ? MIN_PEER_PROTO_VERSION : MIN_PEER_PROTO_VERSION_AFTER_DIFF_HF;
